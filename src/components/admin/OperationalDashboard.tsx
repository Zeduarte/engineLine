...
export async function OperationalDashboard() {
 ...
 const [{data:due,error},{count:unanswered},{count:unassigned}]=await Promise.all([
  db.from("leads").select("id,name,next_action,next_action_at").eq("vehicle_type",vehicleType).in("status",["new","contacted","proposal"]).lte("next_action_at",now).order("next_action_at").limit(10),
  db.from("leads").select("id",{count:"exact",head:true}).eq("vehicle_type",vehicleType).eq("status","new").is("first_contacted_at",null),
  db.from("leads").select("id",{count:"exact",head:true}).eq("vehicle_type",vehicleType).in("status",["new","contacted","proposal"]).is("assigned_to",null),
 ]);
